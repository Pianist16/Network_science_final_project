...
### This script is for the network of type 2nd -> winner, 3rd -> winner ###
### i.e. it is about "absolute winner" of the game, without taking into account the link from player who finished 3rd to player who finished 2nd ###
...
for (i in seq(from = 1, to = 1111, by = 2)){
...
for (i in seq(from = 2, to = 1111, by = 2)){
  if (d1[i,1] > d1[i,2] & d1[i,1] > d1[i,3] & d1[i,2] > d1[i,3]){
    e2.1[i,1] <- b1[i,1] #A
    e2.1[i,2] <- b1[i,2] #B
...
for (i in seq(from = 1, to = 199, by = 2)){
...
r1.1 <- rbind(e1.1,e2.1)
r2.1 <- na.omit(r1.1)
r3.1 <- data.frame(r2.1[,2],r2.1[,1])

r1.2 <- rbind(e1.2,e2.2)
r2.2 <- na.omit(r1.2)
...
w1 <- q1 - 1 # for subgraph size of 3; for period 2001-2015
w2 <- q2 - 1 # for subgraph size of 3; for period 1994-2000
w3 <- q1 - 1 # for subgraph size of 4; for period 2001-2015
w4 <- q2 - 1 # for subgraph size of 4; for period 1994-2000

write.table(w1, file = "w1.txt", sep = " ", row.names = FALSE, col.names = FALSE, append = FALSE)
write.table(w2, file = "w2.txt", sep = " ", row.names = FALSE, col.names = FALSE, append = FALSE)
write.table(w3, file = "w3.txt", sep = " ", row.names = FALSE, col.names = FALSE, append = FALSE)
write.table(w4, file = "w4.txt", sep = " ", row.names = FALSE, col.names = FALSE, append = FALSE)
